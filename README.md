...
### 🔄 Pipeline Engine - 流水线多角色审查引擎

**核心亮点** — 传统 AI 应用一次 LLM 调用就出结果，Prompt Flow Craft 采用 **流水线多阶段审查** 机制，每次生成都经过三个独立角色的 LLM 交叉验证：
...
用户输入
    ...
    ▼
┌─────────────────────────┐
│  S1: Draft Generator    │ ← 第一角色：草稿生成
│  "根据需求生成初始草稿"  │
└──────────┬──────────────┘
           │ 流式输草稿
           ▼
┌─────────────────────────┐
│  S2: Quality Auditor    │ ← 第二角色：质量审查
│  "从5个维度评分审查"     │
│  完整性 / 清晰度 / 结构  │
│  可执行性 / 安全性       │
└──────────┬──────────────┘
...
| 传统方式 | Pipeline 方式 |
|----------|---------------|
| 一次 LLM 调用，直接出结果 | **三次** LLM 调用，三组独立 Prompt |
| 输出的好坏完全靠运气 | **每轮都有质量评分**，低于阈值自动返工 |
| 模型"偷懒"或"幻觉"没人管 | **专职审计角色**专门找茬，确保质量 |
| 无法迭代优化 | **可配置循环次数 + 质量阈值**，灵活控制 |
| 用户看不到中间过程 | **SSE 流式推送每个阶段进度**，透明可见 |

**技术实现：**
```
PipelineOrchestrator              ← 编排器，调度三个阶段
 ├─ PipelineStage                 ← 阶段接口（抽象策略）
 │   ├─ DraftGeneratorStage       ← S1 草稿生成
 │   ├─ QualityAuditStage         ← S2 质量审计（5维评分）
 │   └─ PromptRefineStage         ← S3 精炼优化
 ├─ PipelineContext                ← 跨阶段数据传递
 └─ PipelineConfig                 ← 可配置轮数/阈值/开关
```

> **配置示例**：`application.yml` 中设置 `pipeline.max-rounds: 2`、`pipeline.quality-threshold: 80`，可零代码调整审查严格程度。
