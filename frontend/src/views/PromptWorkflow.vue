          ...
          <el-form :model="form" label-width="100px">
            ...
            <el-form-item label="步骤">
              <div class="steps-editor">
                <div v-for="(step, idx) in form.steps" :key="idx" class="step-item" :class="{ active: editingStepIndex === idx }" draggable="true" @dragstart="onDragStart(idx, $event)" @dragover.prevent @drop="onDragOver(idx)">
                  ...
                  <el-input v-model="step.taskDescription" type="textarea" :rows="3" placeholder="任务描述 (可用 {prev_result} 引用上一步结果)" />
                  <div class="step-var-tip" v-if="idx > 0">💡 可用变量: <code>{prev_result}</code> 代表上一步结果</div>
...
const flowNodes = computed(() => {
  ...
  const nodeW = 120, nodeH = 50, gap = 60
  const totalW = count * nodeW + (count - 1) * gap
  const startX = Math.max(10, (600 - totalW) / 2)
  return form.steps.map((_, idx) => ({ x: startX + idx * (nodeW + gap), y: 20 }))
})

const flowConnections = computed(() => {
  const conns = []
  for (let i = 0; i < form.steps.length - 1; i++) {
    conns.push({ x1: flowNodes.value[i].x + 120, y1: flowNodes.value[i].y + 25, x2: flowNodes.value[i+1].x, y2: flowNodes.value[i+1].y + 25 })
  }
  return conns
})
...
const flowViewBox = computed(() => {
  const count = form.steps.length
  const width = count * 120 + (count - 1) * 60 + 40
  return width + " 0 0 90"
})
  ...
  for (let i = 0; i < runningWorkflow.value.steps.length; i++) {
    ...
    try {
      const response = await fetch('http://111.231.107.210:8080/api/prompt/generate', {
        ...
        body: JSON.stringify({ ...runningWorkflow.value.steps[i], taskDescription: taskDesc })
      })
